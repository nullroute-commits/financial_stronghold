...
class RabbitMQClient:
    ...
    def __init__(self, host: str, port: int, username: str, password: str, virtual_host: str = "/", **kwargs):
        """
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            self.channel.exchange_declare(exchange=f"{self.exchange}.dlx", exchange_type="topic", durable=True)
    ...
    def declare_queue(self, queue_name: str, durable: bool = True, dead_letter: bool = True) -> str:
        """
        Declare a queue.
        ...
        if dead_letter:
            ...
            self.channel.queue_declare(queue=f"{queue_name}.dead", durable=True)

            # Bind the dead-letter queue to the dead-letter exchange
            self.channel.queue_bind(
                exchange=f"{self.exchange}.dlx", queue=f"{queue_name}.dead", routing_key=f"{queue_name}.dead"
            )

        logger.debug(f"Declared queue: {queue_name}")
        return queue_name

    def publish(self, routing_key: str, message: Any, persistent: bool = True, **properties) -> bool:
        """
        Publish a message.
        ...
        try:
            ...
            self.channel.basic_publish(
                exchange=self.exchange, routing_key=routing_key, body=message, properties=pika.BasicProperties(**props)
            )
