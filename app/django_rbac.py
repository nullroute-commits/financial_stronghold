...
class DjangoRBACManager:
    ...
    def create_role(
        self, name: str, description: str = None, permissions: List[str] = None, created_by: Optional[User] = None
    ) -> Optional[str]:
        """
        Create a new role.
        ...
        try:
            ...
            # Create the role
            role = Role.objects.create(name=name, description=description, created_by=created_by)

            # Assign permissions if provided
            if permissions:
                permission_objects = Permission.objects.filter(name__in=permissions, is_active=True)
    ...
    def create_permission(
        self, name: str, resource: str, action: str, description: str = None, created_by: Optional[User] = None
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            permission = Permission.objects.create(
                name=name, resource=resource, action=action, description=description, created_by=created_by
            )
    ...
    def _clear_user_cache(self, user: User):
        ...
        cache_keys = [f"user_permissions:{user.id}", f"user_roles:{user.id}"]

        cache.delete_many(cache_keys)

    def _log_role_action(
        self,
        action: str,
        user: Optional[User],
        role_name: str,
...
def initialize_default_permissions():
    ...
    for perm_data in default_permissions:
        permission, created = Permission.objects.get_or_create(name=perm_data["name"], defaults=perm_data)
        if created:
