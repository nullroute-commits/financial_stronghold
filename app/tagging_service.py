...
class AnalyticsService:
    ...
    def compute_tag_metrics(
        ...
        if resource_type == "transaction":
            return self._compute_transaction_metrics(resource_ids, tenant_type, tenant_id, period_start, period_end)
        elif resource_type == "account":
            return self._compute_account_metrics(resource_ids, tenant_type, tenant_id)
        elif resource_type == "budget":
            return self._compute_budget_metrics(resource_ids, tenant_type, tenant_id)
        else:
            return self._empty_metrics()
    ...
    def _compute_transaction_metrics(
        ...
        return {
            ...
            "currency_breakdown": {
                k: {"count": v["count"], "amount": float(v["amount"])} for k, v in currency_breakdown.items()
            },
            "type_breakdown": {
                k: {"count": v["count"], "amount": float(v["amount"])} for k, v in type_breakdown.items()
            },
            "period_start": period_start.isoformat() if period_start else None,
            "period_end": period_end.isoformat() if period_end else None,
        }

    def _compute_account_metrics(self, resource_ids: List[str], tenant_type: str, tenant_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "type_breakdown": {
                k: {"count": v["count"], "balance": float(v["balance"])} for k, v in type_breakdown.items()
            },
        }

    def _compute_budget_metrics(self, resource_ids: List[str], tenant_type: str, tenant_id: str) -> Dict[str, Any]:
